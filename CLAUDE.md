...
- **A clean check produces no sentence.** (Supersedes the softer 2026-08-10
  wording, which allowed a "materially informative" clean record to stay.) When
  a check ran and came back clear, the document says nothing at all about it -
  no sentence, no citation marker, no list of what was searched, no named
  sources. Delete the line. "No litigation found against the promoter, the
  project or the underlying land in any source reviewed; MahaRERA's complaint,
  appeal and warrant records are empty[18]; the Title Report's 30-year search
  across four courts returned nothing[1]; Form B declares no litigation[2]" is
  three citations and four clauses spent establishing that there is nothing to
  report - it all goes. The scope of what was checked, and the sources that
  established the clean result, stay in the facts file; they do not reach the
  page. **Never attach a citation marker to an absence.**
  Three carve-outs, and only three:
  (a) an **entire section** with nothing in it - see the next rule;
  (b) an absence that is **the stated basis of a score, grade or metric** - see
      "Absence as evidence" below;
  (c) a **gap**, which is an unknown rather than a clean result - see "A gap is
      not an absence" below.
- **An empty section keeps its heading and one bare line.** If a section has no
  findings at all, the heading stays exactly where it is - the flow and the
  section sequence never shift - with the single line "Nothing found."
  underneath it. No sources, no scope, no explanation, no marker. The heading
  plus that line is the whole section.
- **Findings first inside anything that survives.** Where a passage does carry
  real content, state what was established before what is missing. Never open
  on a negative ("Not directly confirmed…", "Not independently obtained…", "No
  discrepancy found.") when the same passage goes on to give real content -
  lead with the content and close with the qualifier. **This applies to table
  cells exactly as it applies to bullets**; a cell is not exempt because it is
  inside a table.
- **Absence as evidence.** An absence survives only where it is the stated
  basis of a number the reader would otherwise have to take on trust: a
  Developer Score sub-metric note ("no completion extension, 0 complaints, 0
  appeals" is why RERA Compliance bands AAA), or a metric card ("Litigation
  Load: 0 complaints / 0 appeals"). The bare figure is a finding; a sentence
  explaining the figure elsewhere in narrative prose is not, and is deleted.
  Strip a score's basis and the grade becomes unauditable - that is the only
  reason this carve-out exists.
- **A gap is not an absence.** "We checked and it is clear" is deleted; "we
  looked and could not establish this" is kept in full. A gap is an open
  unknown the reader can act on, so Gaps & Sources keeps every item with its
  full explanation, including permanent ones that no future pass can close.
  Never compress or delete a gap under the clean-check rule.
- **Deep research on every finding.** Anything that survives as a finding earns
  a dedicated follow-up research pass rather than a one-line mention. A found
  item is written out with what it is, who it involves, when it arose, whether
  it is still live, and what it means for this project. Example: a Notice of
  Lis Pendens is not "a 2017 notice against an adjoining society" - resolve
  which society, under what suit, whether it is still pending, and whether it
  touches this plot's boundary. Budget for this: it is a per-finding research
  stage, not a formatting pass. Space freed by deleting clean-check prose is
  spent here.
- **A live right is a finding.** A contractual permission, option, charge or
  entitlement that exists in a reviewed document counts as found even if it has
  never been exercised, because it remains exercisable. Report the right and
  note that it has not been acted on - do not delete it as "nothing happened".
  (Example: development agreements permitting the developer to mortgage its
  free-sale area, where no mortgage has been taken.) The deep-research pass
  should establish whether the right is still live or has been extinguished.
- **Ruled-out items survive, in the right section.** Something that was
  actually discovered and then dismissed is a finding, not an absence, and it
  does not get deleted. Classify it before deciding where it goes: *found but
  unrelated to this project* stays where it is, compressed to one line with
  the reason it was cleared (e.g. a Lis Pendens naming an adjoining society on
  the same survey number stays under Litigation Status). *Found, related, but
  miscategorised* moves to the section where it is a real fact rather than
  sitting somewhere as a denial (e.g. Deputy Registrar society-amalgamation
  orders are land assembly, not litigation - they belong under Land
  Identification). Age alone never rules an item out.
- **Say it once.** A fact, and an absence, appears in exactly one place. A
  "Summary" line is a verdict plus a pointer - "Insolvency: clean (detail in
  Diligence Appendix)" - never a restatement of the detail it points at. A
  missing document is named once, in Gaps; not again in the section it would
  have supported, and not again in a "not found among the documents reviewed"
  paragraph. Two table rows must not both exist to report the same absence
  (Mortgage area and Mortgage lender both reading "none" is one row's worth of
  information).
- **Flags summarise, Gaps explain.** An item in Overview & Flags is a
  ...
  lives once, under Gaps & Sources, numbered to match. Never print the same
  item in full in both places.
- **Internal keeps process failures, External does not.** Items that record a
  *tooling* failure rather than a fact about the project - a re-verification
  that could not run, an authentication error, a pending manual step - stay in
  full in the Internal document and are cut from the External one. Where such
  an item also contains a genuine finding (for example that a topic rests on a
  single source), preserve the finding in one consolidated line and drop the
  error text. Never put a file path, module name, function or parameter name,
...
- **Deep-dive vs. nothing-found.** Read the heading, not just the bullet. If a
  ...
  material finding sit next to a generic placeholder that undersells it. When
  genuinely nothing was found for an entire section, it collapses to the
  heading plus a single plain "Nothing found" - never padded into multiple
  bullets, and never supported with sources or search scope. Does NOT apply to
  Developer Score or Documentation Confidence Summary - those stay fully intact
  even when mostly N/A (intentional scoring methodology, not a gap to clean
  up), and their N/A notes are covered by "Absence as evidence" above.
  (Amended 2026-08-10: the earlier "a nothing-found statement stays if it is
  materially informative, e.g. a clean record" carve-out is withdrawn. A clean
  record is precisely what now gets deleted.)
- **No jargon, plain language, keep key terms.** Terms like CIRP, NCLT, RP,
  IBC Section 30, DSRA, ECLGS, IGR/e-ASR, KMP, ROC, MCA, IOD, IOA, CHS, NCD,
  ICAI, SRO, SPOC, IBBI, FSI, DCPR, MHADA, MRTP, CC/OC should not appear
  ...
  tribunal (NCLT)"; "Intimation of Disapproval (IOD)"; "the Ministry of
  Corporate Affairs (MCA)"; "cooperative housing society (CHS)") - keep the
  term itself (still searchable/cross-referenceable) but don't require the
  reader to already know it. This is a plain-language pass, not a
  dumbing-down pass - don't lose precision, especially on legal/registry
  facts. The list above is illustrative, not exhaustive: any initialism the
  document introduces is covered.
- **Gloss raw status strings.** A verbatim status value copied out of a
  registry or database (for example IBBI's "ASSIGNMENT NOT APPROVED YET") is
  quoted as-is *and* immediately translated into what it means for this
  promoter, in the same sentence. Never present an opaque database string as
  though it were a finding the reader can interpret.
...
- **Marker placement.** A marker sits at the end of the clause it supports,
  not at the end of the paragraph. A sentence carrying three claims from three
  sources gets three markers, each after its own clause - `[1][2][4]` parked
  after the final full stop is exactly the failure this rule exists to
  prevent. A marker also never lands mid-word or mid-token
  ("MahaRERA[10]-registered"); move it to the end of the clause.
- **Scope of the rule.** "Every inline factual claim" means every one:
  Executive Summary bullets, flag headlines, table cells, appendix prose and
  score-table notes are all in scope, not just narrative body paragraphs. A
  section that currently ships with no markers at all is a defect, not a
  stylistic choice. As of 2026-08-10 roughly two thirds of External bullets
  carried no marker - treat full coverage as the target and check it, don't
  assume it.
- **The marker must actually support the claim.** Cite the source that
  establishes the specific statement. Citing the MahaRERA complaints/appeals
  record for a sentence that begins "independent web research found…" is a
  mis-citation even though both are real sources in the list. Where a claim
  rests on the absence of a result from a named search, cite that search.
- **Source labels.** External Sources entries are descriptive citations -
  issuer, what the document is, and its date ("Affidavit-cum-Declaration
  (Form B) under Rule 3(6) - notarized 12-13 March 2024, filed with
  MahaRERA"). Never a raw internal filename, never a bare category label like
  "Project record" or "appeals data" that a reader cannot check. Internal keeps
  its own full bibliographic list with filenames; that list is not reused for
  External.
